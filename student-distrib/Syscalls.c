...
int32_t vidmap (uint8_t** screen_start) {
    ...
    PCB_t * p = get_PCB();
    PCB_t * pp = get_PCB_withpid(p->parent_pid);
    SET_PD_ENTRY_4K(PD[vm>>22], &PT_user[0], 1, 1);
    if (pp->visible) SET_PT_ENTRY(PT_user[(vm>>12)&(0x3FF)], 0xB8000, 1, 1);
    else SET_PT_ENTRY(PT_user[(vm>>12)&(0x3FF)], BackupVP(pp->pid), 1, 1);
